...
import defaultBiscuitState from './default-biscuit-state';
...
function getComponentModels() {
  if (_.isEmpty(_.get(window, 'kiln.componentModels'))) {
    ...
    Object.keys(window.modules)
      ...
      .forEach(key => {
        window.kiln.componentModels[key.replace('.model', '')] = window.require(
          key
        );
      });
  }
...
function getComponentKilnjs() {
  ...
  if (_.isEmpty(_.get(window, 'kiln.componentKilnjs'))) {
    ...
    Object.keys(window.modules)
      ...
      .forEach(key => {
        window.kiln.componentKilnjs[key.replace('.kiln', '')] = window.require(
          key
        );
      });
  }
...
export function biscuitPreloader({ commit, dispatch }) {
  const schemas = _.get(window, 'kiln.preloadSchemas'),
    locals = _.get(window, 'kiln.locals'),
    models = getComponentModels(),
    componentData = _.get(window, 'kiln.componentData', {}),
    componentKilnjs = getComponentKilnjs(),
    templates = _.get(window, 'kiln.componentTemplates'),
    tabulaRasa = _.assign({}, defaultBiscuitState),
    site = locals && normalizeSiteData(locals.site),
    normalizedComponents = _.reduce(componentData, reduceComponents, {});
  ...
  return props({
    ...
    dispatch('finishProgress', 'draft'); // draft for now...
  })
    ...
    ({ pageState, layoutState, pageData, allSites, favoritePageCategory }) => {
      ...
      commit(
        ...
        _.assign(tabulaRasa, {
          ...
          layout: _.assign({}, data._layoutData, {
            state: layoutState,
